...
export const MeteorSpell: SpellBaseType = createSpell({
  ...
  getDescription({ spellInstance }) {
    ...
    return {
      descriptions: [
        spellDescrElem(
          `Meteor deals ${minDamage + damageBounsPerLevel}-${maxDamage + damageBounsPerLevel} fire damage to random enemy group, next 2 unit groups after current group in fight queue (enemies or allies) will be stunned and will lose their turns. If stunned enemy's creature is tier 4 or higher, instead of stun, it will take 65% of ability's damage.`,
        ),
      ],
    };
  },
    ...
    init({ events, actions, thisSpell, ownerHero, vfx, spellInstance }) {
      events.on({
        PlayerCastsInstantSpell(event) {
          ...
          const meteorDamage = CommonUtils.randIntInRange(
            minDamage + damageBounsPerLevel,
            maxDamage + damageBounsPerLevel,
          );
          ...
          actions.dealDamageTo(
            ...
            ({ unitLoss, initialUnitCount, finalDamage }) => {
              actions.historyLog(
                `${ownerHero.name} deals ${finalDamage} damage to ${initialUnitCount} ${randomEnemyGroup.type.name} with ${thisSpell.name}, ${unitLoss} units perish`,
              );

              vfx.createFloatingMessageForUnitGroup(randomEnemyGroup, getDamageParts(finalDamage, unitLoss), {
                duration: 1000,
              });
            },
          );
            ...
            if (unit.type.level < 4) {
              ...
              if (actions.isEnemyUnitGroup(unit)) {
                actions.dealDamageTo(
                  ...
                  ({ unitLoss, initialUnitCount, finalDamage }) => {
                    actions.historyLog(
                      `${ownerHero.name} deals ${finalDamage} damage to ${initialUnitCount} ${unit.type.name} with ${thisSpell.name}, ${unitLoss} units perish`,
                    );
