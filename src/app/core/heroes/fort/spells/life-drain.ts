...
export const LifeDrainSpell = createSpell({
  ...
  getDescription: ({ spellInstance }) => ({
    descriptions: [
      spellDescrElem(`Reallocates health from enemies, dealing ${baseValue + levelBonus * (spellInstance.currentLevel - 1)} damage to all enemies and equally distributing the dealt damage as healing among your army.`),
    ]
  }),
  config: {
    spellConfig: {
