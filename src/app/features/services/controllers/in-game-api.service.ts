...
import { ItemsEventsHandlers, ItemsEventsGroup, ItemEventTypes } from 'src/app/core/items';
import { PlayerInstanceModel } from 'src/app/core/players';
import { SpellEventHandlers, SpellEventsGroup, SpellEventTypes, SpellInstance, SpellModel } from 'src/app/core/spells';
      ...
      events: {
        on: (handlers: SpellEventHandlers) => {
          Object.entries(handlers).forEach(([eventName, handler]) => {
            const event = SpellEventsGroup.getEventByName(eventName as SpellEventTypes);
      ...
      events: {
        on: (newEventHandlers: ItemsEventsHandlers) => {
          Object.entries(newEventHandlers).forEach(([eventName, handler]) => {
            const event = ItemsEventsGroup.getEventByName(eventName as ItemEventTypes);
